...
function updateUI(jobAnouncements) {
  ...
  jobAnouncements.forEach(announcement => {
    ...
    const { title, coverPhoto, published, shift, place, tags, isFeatured, isNew, companyName, id } = announcement;
    let taghtml = "";
    tags.forEach(tag => {
    ...
    jobCardElement.setAttribute('class', `job-card ${isNew && isFeatured ? "job-card--left" : ""}`)
    ...
    ${isNew ? `<span class="job-card__badge job-card__badge--new">New</span>` : ""}
    ${isFeatured ? ` <span class="job-card__badge job-card__badge--featured" > Featured</span >` : ""}
      </div>
...
container.addEventListener('click', function (e) {
  ...
  if (targetElement.classList.contains('job-card__tag')) {
    ...
    const anouncements = filterJobAnouncements(selectedTags, jobAnouncements)
